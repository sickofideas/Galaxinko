// --- GALAXINKO (v6.2.1 - VERTICAL 9:14 & CLEAN PANELS) ---
// Vychází ze stabilní verze v6.2.0.
// ÚPRAVY: Změna rozlišení na 900x1400 (9:14). Ztmavené a graficky upravené boční panely pro snazší čtení.
...
function spawnSpaceship() {
    let shipW = 140; let shipH = 25;
    let shipY = H - ZONE_H - 150;
...
function draw() {
  ...
  stroke(currentTheme[0], currentTheme[1], currentTheme[2], 140); strokeWeight(2);
...
function drawBalls() {
  for (let i = balls.length - 1; i >= 0; i--) {
    ...
    if (isNaN(pos.x) || isNaN(pos.y)) { removeBall(b);
    continue; }
    ...
    rotate(b.body.angle);
    if (b.combo > 2) {
        fill(255); noStroke(); rect(-7, -7, 14, 14);
    }
    
    fill(b.color); stroke(255); strokeWeight(1); rect(-5, -5, 10, 10);
    rotate(-b.body.angle);
    fill(0, 150); noStroke(); textAlign(CENTER); textSize(11);
    text(b.name, 1, -13); // Stín
    fill(b.color); text(b.name, 0, -14);
    if (b.combo > 0) { 
        fill(0, 150); textSize(13);
        text("x" + b.combo, 1, -25); // Stín
        fill(255, 200, 0);
        text("x" + b.combo, 0, -26); 
    }
    pop();
    ...
    if (starship && starship.state === "ACTIVE") {
        if (abs(pos.x - starship.body.position.x) < starship.w/2 + 10 && abs(pos.y - starship.y) < starship.h/2 + 10) {
            if (millis() - (b.lastShipHit || 0) > 500) {
                ...
                b.combo += 2; b.lastHitTime = millis();
    ...
    for (let j = pegs.length - 1; j >= 0; j--) {
      ...
      if (dist(pos.x, pos.y, p.position.x, p.position.y) < 14) {
          p.glow = 255;
          b.combo += 1; b.lastHitTime = millis();
          if (p.isExplosive) {
...
function drawViewerObjects() {
  ...
  for (let obj of viewerSpaceObjects) {
    ...
    obj.y += obj.vy + cos(frameCount * 0.01) * 0.1; obj.angle += 0.005;
    ...
    else { fill(obj.color[0], obj.color[1], obj.color[2], obj.alpha); ellipse(0, 0, totalS); fill(255, obj.alpha); textAlign(CENTER, CENTER); textSize(totalS * 0.3);
    text(obj.name[0], 0, 0); }
...
function drawUI() {
  ...
  fill(currentTheme[0], currentTheme[1], currentTheme[2], 20); textSize(70); text(GAME_TITLE, logoX + 4, logoY + 4);
  fill(255); textSize(70); text(GAME_TITLE, logoX, logoY);
  fill(currentTheme[0], currentTheme[1], currentTheme[2]);
  textSize(14); text("STABLE SINGULARITY SIMULATION v6.3.0", logoX + 2, logoY + 38);
  ...
  fill(5, 5, 20, 250);
  stroke(currentTheme[0], currentTheme[1], currentTheme[2], 120 + pulse * 10); strokeWeight(2); rect(dropZoneX, 10, dropZoneW, 75, 12);
  noStroke(); textAlign(CENTER, CENTER); fill(255); textSize(18);
  text("SYSTEM STATUS: ONLINE", dropZoneX + dropZoneW/2, 35);
  ...
  let gDisp = floor(map(currentGravity, 0.05, 1.95, 1, 99)); fill(200); textSize(10);
  text(`G-FORCE: ${gDisp} [R-${roundCount}]`, W - 25, 50);
  fill(255, 150, 0); text(`BOUNCE-X: ${currentBounce}`, W - 25, 68);
  pop();
  
  // VYLEPŠENÝ LEVÝ PANEL (Mission Milestones)
  push(); translate(15, 115);
  fill(0, 0, 15, 230); stroke(currentTheme[0], currentTheme[1], currentTheme[2], 120); strokeWeight(2);
  rect(0, 0, 280, 260, 8);
  noStroke(); fill(currentTheme[0], currentTheme[1], currentTheme[2]); textAlign(CENTER); textSize(13); text("MISSION MILESTONES", 140, 25);
  textAlign(LEFT);
  allTimeRecords.forEach((rec, i) => {
    let tSize = (i === 0) ? 14 : 12; textSize(tSize);
    fill(rec.color[0], rec.color[1], rec.color[2]); text(`${i+1}. ${rec.name}`, 15, 55 + i * 25);
    textAlign(RIGHT); fill(255, 180); text(rec.score, 265, 55 + i * 25); textAlign(LEFT);
  });
  
  translate(0, 275);
  fill(0, 0, 15, 230); stroke(currentTheme[0], currentTheme[1], currentTheme[2], 120); strokeWeight(2); rect(0, 0, 280, 70, 8); noStroke();
  textSize(11);
  if (gameState === "PLAYING") {
    textAlign(LEFT, CENTER); fill(timer < 10 ? color(255,50,50) : color(currentTheme[0], currentTheme[1], currentTheme[2]));
    text("WARP-DRIVE: " + timer + "s", 15, 25); fill(50, 255, 50); text(`ACTIVE UNITS: ${totalBallsFired}`, 15, 50);
  } else if (gameState === "WAITING") {
    textAlign(LEFT, CENTER); fill(255, 200, 0); text("COOLING DOWN...", 15, 25);
    fill(50, 255, 50); text(`TOTAL UNITS: ${totalBallsFired}`, 15, 50);
  }
  pop();
  
  // VYLEPŠENÝ PRAVÝ PANEL (Top Contributors)
  push(); translate(W - 295, 115);
  let sorted = Object.entries(leaderboard).sort((a, b) => b[1].score - a[1].score).slice(0, 12);
  fill(0, 0, 15, 230); stroke(currentTheme[0], currentTheme[1], currentTheme[2], 120); strokeWeight(2); 
  rect(0, 0, 280, 320, 8);
  noStroke(); fill(currentTheme[0], currentTheme[1], currentTheme[2]); textAlign(CENTER); textSize(13);
  text("TOP CONTRIBUTORS", 140, 25);
...
function mouseClicked() {
  ...
  if (mouseX > W-295 && mouseX < W && mouseY > 115 && mouseY < 435) { leaderboard = {};
  shakeAmount = 4; return; }
  if (mouseX > 15 && mouseX < 295 && mouseY > 115 && mouseY < 155) {
  ...
  if (mouseY > 0 && mouseY < 110) { spawnBall(random(TEST_BOTS));
  shakeAmount = 2; }
}
...
function drawExplosions() {
  for (let i = explosions.length - 1; i >= 0; i--) {
    let e = explosions[i];
    fill(red(e.col), green(e.col), blue(e.col), e.life); rect(e.x, e.y, 4, 4);
...
function initGame() {
  ...
  if (mode.startsWith("SHAPE_")) {
    ...
    let cols = shape[0].length; let spacing = 26;
    ...
    for (let i = 0; i < 200; i++) {
      ...
      let py = random(140, H - 300); let valid = true;
...
function drawZones() {
  for (let z of zones) {
    ...
    push();
    translate(z.x + z.w/2, H - 20); rotate(-HALF_PI); textAlign(LEFT, CENTER);
    // ZVĚTŠENÉ TEXTY V ZÓNÁCH
    if (isJackpot) { fill(255, 230, 100);
    textSize(16); text(z.score, 0, 0); }
...
function drawWaitingMessage() {
  ...
  textSize(14); noStroke();
  fill(255, 200, 0, alpha); text("REMAINING UNITS RETURNING TO BASE...", W/2, H/2); pop();
}

function drawResultsOverlay() {
  fill(0, 0, 20, 230);
  rect(20, 50, W - 40, H - 100, 20);
  stroke(currentTheme[0], currentTheme[1], currentTheme[2], 150); strokeWeight(4); noFill();
  rect(30, 60, W - 60, H - 120, 15);
  // ZVĚTŠENÉ TEXTY NA VÝSLEDKOVÉ OBRAZOVCE
  noStroke(); fill(currentTheme[0], currentTheme[1], currentTheme[2]);
  textAlign(CENTER); textSize(55); text("ROUND COMPLETE", W/2, 140);
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    textAlign(LEFT, CENTER); fill(entry[1].color); textSize(40);
...
function drawProceduralHUD() {
  ...
  textAlign(RIGHT); text(`SENS_TEMP: ${(24 + noise(frameCount*0.01)*5).toFixed(1)}°C`, W - 20, H - 30);
...
function triggerCosmicEvent() {
  ...
  let size = random(25, 45); let startX = fromLeft ? -100 : W + 100;
