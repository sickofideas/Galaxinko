// --- GALAXINKO (v6.2.1 - 9:11 RATIO & FIXED HUD) ---
// NOVINKY:
// • Rozlišení upraveno na 900x1100 (poměr 9:11)
// • Opravena překrývající se horní lišta – štítek LIVE a SYSTEM STATUS jsou nyní čistě srovnané.
...
function getHoustonStory(playerName) {
  ...
  const outros = ["Keep it up on the live stream!", "Awaiting further instructions.", "Over and out.", "May the space force be with you."];
    ...
    try {
      ...
      if (possibleName && possibleName !== "Anonym") {
        ...
        if (evt === "like") {
          ...
          if (millis() - lastSpokeTime > 9000) { speakAnnouncer(random([`Houston to ${safeName}, energy shields are boosted thanks to your transmission!`, `Live feed confirms power up from ${safeName}.`]), 0); lastSpokeTime = millis(); }
          for (let i = 0; i < count; i++) {
            setTimeout(() => { for (let s = 0; s < spawnPerEvent; s++) { spawnBall(name); } }, i * 120);
          }
        }
      }
    } catch (err) {}
...
function draw() {
  ...
  if (settingsPanelVisible) {
    push();
    fill(0, 0, 30, 220); rect(20, 130, 300, 290, 12);
    fill(currentTheme[0], currentTheme[1], currentTheme[2]); textSize(14); text("TEST SETTINGS 🔧", 40, 155);
    textSize(11); fill(255);
    text(`GRAVITY (${currentGravity.toFixed(2)})`, 40, 195);
    text(`BOUNCE (${currentBounce})`, 40, 245);
    text(`SPAWN PER JOIN (${spawnPerEvent})`, 40, 295);
    text(`SHIP SPAWN % (${currentShipChance})`, 40, 345);
    pop();
  }
...
function spawnBall(userName) {
  ...
  let ballBody = Matter.Bodies.rectangle(spawnX, 130, 10, 10, {
...
function onUserJoin(username, imgUrl) {
  ...
  let obj = {
    name: username, x: random(100, W-100), y: random(150, H-300), vx: random(-0.3, 0.3), vy: random(-0.3, 0.3),
...
function drawViewerObjects() {
  ...
  for (let obj of viewerSpaceObjects) {
    ...
    if (obj.x < 50 || obj.x > W-50) obj.vx *= -1; if (obj.y < 120 || obj.y > H-250) obj.vy *= -1;
...
function drawUI() {
  push();
  // OPRAVENÁ HORNÍ LIŠTA PROTI PŘEKRÝVÁNÍ
  fill(0, 0, 30, 255); noStroke(); rect(0, 0, W, 110);
  stroke(0, 255, 255, 100); strokeWeight(2); line(0, 108, W, 108);
  
  let logoX = 20, logoY = 40;
  textAlign(LEFT, CENTER); 
  fill(0, 255, 255, 20); textSize(56); text(GAME_TITLE, logoX + 4, logoY + 4);
  fill(255); textSize(56); text(GAME_TITLE, logoX, logoY);
  fill(0, 255, 255); textSize(10); text("STABLE SINGULARITY SIMULATION v6.2.1", logoX + 2, logoY + 40);
  
  let dropZoneW = 380, dropZoneX = W/2 - (dropZoneW / 2);
  let pulse = sin(frameCount * 0.1) * 3;
  fill(0, 255, 255, 10 + pulse); rect(dropZoneX - 10, 10, dropZoneW + 20, 90, 15);
  fill(5, 5, 20, 250); stroke(0, 255, 255, 120 + pulse * 10); strokeWeight(2); rect(dropZoneX, 15, dropZoneW, 80, 12);
  
  noStroke(); textAlign(CENTER, CENTER); 
  fill(255); textSize(15); text("SYSTEM STATUS: ONLINE", dropZoneX + dropZoneW/2, 35);
  
  let liveTime = new Intl.DateTimeFormat('cs-CZ', { timeZone: 'Europe/Prague', dateStyle: 'short', timeStyle: 'medium' }).format(new Date());
  fill(255, 50, 50); textSize(12); text("🔴 LIVE " + liveTime, dropZoneX + dropZoneW/2, 58);
  
  fill(0, 255, 255); textSize(9); text("GEOMETRY: PROCEDURAL | DATA: SYNCED", dropZoneX + dropZoneW/2, 78);
  
  fill(0, 255, 255); textAlign(RIGHT); textSize(10); text(`${currentDestination}`, W - 25, 35);
  let gDisp = floor(map(currentGravity, 0.05, 1.95, 1, 99)); fill(200); textSize(9); text(`G-FORCE: ${gDisp} [R-${roundCount}]`, W - 25, 58);
  fill(255, 150, 0); text(`BOUNCE-X: ${currentBounce}`, W - 25, 78);
  pop();
  
  // ZTMAVENÉ BOČNÍ PANELY S LEPŠÍ ČITELNOSTÍ
  push(); translate(0, 120);
  fill(0, 0, 15, 240); stroke(0, 255, 255, 50); strokeWeight(2); rect(10, 0, 260, 235, 6);
  noStroke(); fill(0, 255, 255); textAlign(CENTER); textSize(10); text("MISSION MILESTONES", 140, 20);
  stroke(255, 20); strokeWeight(1); line(20, 30, 260, 30); noStroke();
  
  textAlign(LEFT);
  allTimeRecords.forEach((rec, i) => {
    let tSize = (i === 0) ? 12 : (i === 1) ? 10 : 9; textSize(tSize);
    fill(rec.color[0], rec.color[1], rec.color[2]); text(`${i+1}. ${rec.name}`, 22, 50 + i * 22);
    textAlign(RIGHT); fill(255, 200); text(rec.score, 260, 50 + i * 22); textAlign(LEFT);
  });
  
  translate(0, 245);
  fill(0, 0, 15, 240); stroke(0, 255, 255, 50); strokeWeight(2); rect(10, 0, 260, 65, 6); noStroke();
  textSize(9);
  if (gameState === "PLAYING") {
    textAlign(LEFT, CENTER); fill(timer < 10 ? color(255,50,50) : color(0,255,255));
    text("WARP-DRIVE: " + timer + "s", 22, 20); fill(50, 255, 50); text(`ACTIVE UNITS: ${totalBallsFired}`, 22, 45);
  } else if (gameState === "WAITING") {
    textAlign(LEFT, CENTER); fill(255, 200, 0); text("COOLING DOWN...", 22, 20); fill(50, 255, 50); text(`TOTAL UNITS: ${totalBallsFired}`, 22, 45);
  }
  pop();
  
  push(); translate(W - 270, 120);
  let sorted = Object.entries(leaderboard).sort((a, b) => b[1].score - a[1].score).slice(0, 12);
  fill(0, 0, 15, 240); stroke(0, 255, 255, 50); strokeWeight(2); rect(0, 0, 260, 300, 6);
  noStroke(); fill(0, 255, 255); textAlign(CENTER); textSize(10); text("TOP CONTRIBUTORS", 130, 20);
  stroke(255, 20); strokeWeight(1); line(10, 30, 250, 30); noStroke();
  
  textAlign(LEFT); textSize(9);
  sorted.forEach((e, i) => {
    fill(e[1].color); text(`${nf(i+1, 2)}. ${e[0]}`, 15, 55 + i * 20);
    textAlign(RIGHT); fill(255); text(e[1].score, 245, 55 + i * 20); textAlign(LEFT);
  });
  pop();
}
...
function mouseClicked() {
  if (!audioStarted) startSpaceAudio();
  if (mouseX > W-260 && mouseX < W && mouseY > 120 && mouseY < 420) { leaderboard = {}; shakeAmount = 4; return; }
  if (mouseX > 10 && mouseX < 270 && mouseY > 120 && mouseY < 150) {
  ...
  if (mouseY > 0 && mouseY < 110) { spawnBall(random(TEST_BOTS)); shakeAmount = 2; }
}
...
function drawResultsOverlay() {
  ...
  stroke(0, 255, 255, 150); strokeWeight(4); noFill(); rect(30, 60, W - 60, H - 120, 15);
  noStroke(); fill(0, 255, 255); textAlign(CENTER); textSize(45); text("ROUND COMPLETE", W/2, 140);
