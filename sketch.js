// --- GALAXINKO (v8.5.0 - ANTI-BAN AVATAR & NEON UI) ---
...
function startSpaceAudio(){if(audioStarted)return;userStartAudio();backgroundOsc.start();backgroundOsc.amp(0.02,2);backgroundOsc.freq(55);backgroundOsc2.start();backgroundOsc2.amp(0.01,2);backgroundOsc2.freq(110);bhOsc.start();bhOsc.amp(0);audioStarted=true;speakAnnouncer("Houston to ground control.",2);}
...
function handleFloatingTexts() {
    for (let i = floatingTexts.length - 1; i >= 0; i--) {
        ...
        stroke(0, ft.life); strokeWeight(3);
...
function draw(){
  ...
  if(gameState==="WAITING"){
      if(balls.length===0||(millis()-waitStartTime)/1000>10){
          ...
          if(s.length>0) speakAnnouncer(`Round over! The ultimate commander is ${sanitizeText(s[0][0])}.`,2);
      }
  }
  
  drawPortals(); drawZones();drawWalls();drawPegs();drawBalls();drawExplosions();handleFollowEvents();
  handleShockwaves(); handleFloatingTexts(); 
...
function drawBalls(){
  ...
  for(let i=balls.length-1;i>=0;i--){
    ...
    noStroke();
    if(b.isRainbow){
        colorMode(HSB, 360, 100, 100);let c = color((frameCount*10)%360, 100, 100);colorMode(RGB, 255, 255, 255);
        fill(red(c), green(c), blue(c), 100); rect(-10,-10,20,20); fill(c);
    } else {
        fill(red(b.color), green(b.color), blue(b.color), 100); rect(-10,-10,20,20); fill(b.color);
    }
...
function drawUI(){
  ...
  textAlign(RIGHT,CENTER);fill(currentTheme[0],currentTheme[1],currentTheme[2]);textSize(12);text(`${currentDestination}`,W-15,20);let gDisp=floor(map(currentGravity,0.05,1.95,1,99));fill(200);textSize(10);text(`G-FORCE: ${gDisp} [R-${roundCount}]`,W-15,38);fill(255,150,0);text(`BOUNCE-X: ${currentBounce}`,W-15,56);pop();
  ...
  ml.forEach((r,i)=>{
    textSize(i<3?13:10);let y=65+i*26;let txt=`${i===0?'👑 ':''}${i+1}. ${r.name}`;
    fill(0); text(txt,15+2,y+2); textAlign(RIGHT); text(r.score.toLocaleString(),225+2,y+2);
    textAlign(LEFT); fill(r.color[0],r.color[1],r.color[2]); text(txt,15,y);
    textAlign(RIGHT); fill(255); text(r.score.toLocaleString(),225,y); textAlign(LEFT);
  });
  ...
  sorted.forEach((e,i)=>{
    textSize(i<3?13:10);let y=65+i*22;let txt=`${i===0?'👑 ':''}${nf(i+1,2)}. ${e[0]}`;
    fill(0); text(txt,15+2,y+2); textAlign(RIGHT); text(e[1].score.toLocaleString(),225+2,y+2);
    textAlign(LEFT); fill(e[1].color); text(txt,15,y);
    textAlign(RIGHT); fill(255); text(e[1].score.toLocaleString(),225,y); textAlign(LEFT);
  });
  pop();
}
...
function drawResultsOverlay(){
  ...
  for(let i=0;i<sorted.length;i++){
    ...
    fill(0); text(`${i===0?"👑 ":i+1+". "}${entry[0]}`, 100+2, yPos-5+2);
    fill(entry[1].color); textSize(i===0?45:35); text(`${i===0?"👑 ":i+1+". "}${entry[0]}`,100,yPos-5);
    textAlign(RIGHT,CENTER); fill(0); text(entry[1].score.toLocaleString(), W-100+2, yPos-5+2);
    fill(255); textSize(i===0?45:38); text(entry[1].score.toLocaleString(),W-100,yPos-5);
  }
...
function drawPixelAvatar(x, y, w, h) {
  ...
  let vlasy = color(240, 220, 110); let kuze = color(245, 200, 170);
  let triko = color(currentTheme[0], currentTheme[1], currentTheme[2]);
  let stin = color(0, 0, 0, 50); let oci = color(40);
  let pw = w / 20; let ph = h / 15;
  let sway = sin(frameCount * 0.05) * 2;
  let handMove = sin(frameCount * 0.2) * 4;
  let eyesOpen = (frameCount % 120 > 5);
  ...
  fill(vlasy);
  rect(pw*5 + headSway, ph*1, pw*10, ph*3, 2);
  rect(pw*5 + headSway, ph*3, pw*2, ph*7, 2);
  rect(pw*13 + headSway, ph*3, pw*2, ph*7, 2);
  ...
  fill(kuze);
  rect(pw*3, ph*8 + handMove, pw*3, ph*4, 2);
  rect(pw*14, ph*8 - handMove, pw*3, ph*4, 2);
  pop();
}

function drawAntiBotOverlay(){
  push();
  stroke(0, 15); strokeWeight(1); let offset = frameCount % 4;
  for(let i = 0; i < H; i += 4){ line(0, i + offset, W, i + offset); }
  ...
  let camX = W - 110; let camY = H - 115;
  fill(10, 10, 10, 200); stroke(50); strokeWeight(2); rect(camX, camY, 100, 75, 5);
  ...
  for(let cx = 0; cx < 100; cx += 5) {
      for(let cy = 0; cy < 75; cy += 5) {
          ...
          if (n > 0.6) { fill(255, 255, 255, 30); rect(camX + cx, camY + cy, 5, 5); }
      }
  }
  
  fill(255, 50, 50); textFont('Courier New'); textStyle(BOLD); textSize(10); textAlign(LEFT, TOP); 
  ...
  let scrollX = W - ((frameCount * 3) % (textWidth(marqueeText))); 
  fill(5, 5, 15, 230); noStroke(); rect(0, H - 25, W, 25);
  fill(currentTheme[0], currentTheme[1], currentTheme[2]); textSize(11); textAlign(LEFT, CENTER);
  text(marqueeText + marqueeText, scrollX, H - 12);
  pop();
}
