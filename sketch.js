...
// --- Safe localStorage ---
function safeGet(key, fallback) { try { const v = localStorage.getItem(key); return v ? JSON.parse(v) : fallback; } catch { return fallback; } }
...
function drawViewerObjects() {
  ...
  for (let i = viewerSpaceObjects.length - 1; i >= 0; i--) {
	...
	obj.extraSize *= 0.995; if (obj.extraSize < 0.05) obj.extraSize = 0; if (obj.comboTTL>0) obj.comboTTL--; else obj.combo = max(0, obj.combo - 0.3);
	...
	noStroke(); fill(obj.color[0], obj.color[1], obj.color[2], (18 + 40*comboBoost + 12*haloPulse) * obj.fadeAlpha * overlapFactor);
	ellipse(0,0, totalS + 26 + comboBoost*14);
	for (let k=obj.pulses.length-1; k>=0; k--) {
  	const p = obj.pulses[k]; noFill();
  	stroke(red(p.col), green(p.col), blue(p.col), p.alpha * overlapFactor * obj.fadeAlpha); strokeWeight(2); ellipse(0,0,p.r);
  	p.r += 3.2; p.alpha -= 6; if (p.r > p.maxR || p.alpha <= 0) obj.pulses.splice(k,1);
	}
	const sparks = min(10, 1 + floor(obj.combo/8));
	for (let s=0;s<sparks;s++) {
  	const a = (frameCount*0.05 + s*(TWO_PI/sparks)); const rr = totalS*0.6 + 6 + s*0.6;
	...
	pop();
	rotate(-obj.angle); fill(255, 200 * obj.fadeAlpha * overlapFactor); textSize(10); textAlign(CENTER); text(obj.name, 0, totalS/2 + 15);
	for (let b=obj.popups.length-1; b>=0; b--) {
  	const bb = obj.popups[b]; push(); translate(bb.x, bb.y);
  	noStroke(); fill(red(bb.col), green(bb.col), blue(bb.col), bb.alpha * obj.fadeAlpha * overlapFactor); textAlign(CENTER,CENTER); textSize(12); text(bb.text,0,0);
  	pop(); bb.y += bb.vy; bb.alpha -= 4; if (bb.alpha <= 0) obj.popups.splice(b,1);
	}
	pop();
  }
}
...
function drawLiveClockBadge(centerX, y) {
  ...
  const hh = String(now.getHours()).padStart(2,'0'); const mi = String(now.getMinutes()).padStart(2,'0'); const ss = String(now.getSeconds()).padStart(2,'0');
  ...
  const totalW = wLabel + gap + wTime + pad*2; const rx = centerX - totalW/2; const ry = y - 12; const rh = 24;
  noStroke(); fill(5,5,20,220); rect(rx, ry, totalW, rh, 12);
  fill(255,60,60); circle(rx + pad - 6, y, 6);
  fill(255,60,60); text(label, rx + pad, y);
  fill(220); text(timeStr, rx + pad + wLabel + gap, y);
}
...
function drawUI() {
  ...
  textSize(8);
  if (gameState === "PLAYING") { textAlign(LEFT, CENTER); fill(timer<10?color(255,0,0):color(0,255,255)); text("WARP-DRIVE: " + timer + "s", 22, 18); fill(0,255,0); text(`ACTIVE UNITS: ${totalBallsFired}`, 22, 42); }
  else if (gameState === "WAITING") { textAlign(LEFT, CENTER); fill(255,200,0); text("COOLING DOWN...", 22, 18); fill(0,255,0); text(`TOTAL UNITS: ${totalBallsFired}`, 22, 42); }
  pop();
...
function mouseClicked() {
  ...
  if (mouseX>10 && mouseX<260 && mouseY>100 && mouseY<130) {
	allTimeRecords = Array(8).fill({ name:"NONE", score:0, color:[100,100,100] }); safeSet('galaxinko_records', allTimeRecords);
...
function initGame() {
  ...
  let curX = 0; zones = []; for (let i=0;i<21;i++){
...
function drawResultsOverlay() {
  fill(0, 0, 20, 235); rect(20, 50, W - 40, H - 100, 20);
  noFill(); stroke(0, 255, 255, 160); strokeWeight(3); rect(30, 60, W - 60, H - 120, 16);

  noStroke(); textAlign(CENTER);
  fill(0,255,255); textSize(44); text("ROUND COMPLETE", W/2, 135);
  fill(255, 215, 0); textSize(22); text(`SECTOR: ${currentDestination}`, W/2, 175);
  ...
  if (sorted.length > 0) {
	...
	for (let i=0;i<sorted.length;i++){
  	...
  	fill(0,255,255,180); rect(bx, by, barW, bh, 6);
  	fill(255,255,255,60); rect(bx, by, barW, 3, 6);
	}
  }

  textAlign(CENTER); fill(255, 80, 80); textSize(20); text(`NEXT ROUND IN: ${resultsTimer}s`, W/2, H - 60);
}
...
function drawGalacticBackground() {
  ...
  for(let s of stars){ ellipse(s.x,s.y,s.s); s.y += s.speed * currentTravelSpeed * 5; if (s.y>H){ s.y=0; s.x=random(W);} }
...
function drawDebrisUFO(d){
  ...
  fill(180, 220); ellipse(0, -s/6, s*0.55, s*0.55);
...
function drawDebrisRocket(d){
  ...
  fill(255, 255, 255, 200); triangle(0, -s*0.65, -s*0.18, -s*0.45, s*0.18, -s*0.45);
...
function drawDebrisProbe(d){
  ...
  fill(150, 200); triangle(-s*0.25, -s*0.3, s*0.25, -s*0.3, 0, -s*0.6);
...
function handleSpaceship() {
  ...
  fill(255, 255, 255, 160); rect(-42, -4, 10, 8); rect(32, -4, 10, 8);
...
function resetGame() {
  ...
  gameState = "PLAYING"; resultsTimer = 10; eventOccurredThisRound = false;
  currentDestination = generatePlanetName();
