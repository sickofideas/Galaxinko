// --- GALAXINKO (v5.5.0 - EXTREME ADMIN, COMBO SYSTEM & EXPLOSIVE PEGS) ---
// Opraveno pro TikFinity: spawn při eventu "roomUser"
// NOVINKY v5.5.0:
// • Extrémní limity v admin panelu (Bounce až 200, Spawn až 50).
// • Opravené tlačítko AUTO, které skutečně drží automatický režim po každém kole.
// • COMBO systém: Kuličky nabírají kombo za hity do kolíčků = násobič skóre.
// • VÝBUŠNÉ PEGY: 5 % kolíčků je oranžových, při zásahu explodují a odstřelí kuličku.
...
function toggleAutoMode() {
  ...
  if (isAutoMode) {
    ...
    autoButton.style('background-color', '#4CAF50'); // Zelená pro aktivní stav
    autoRandomSettings();
  } else {
...
function autoRandomSettings() {
  currentGravity = random(0.05, 1.95);
  currentBounce = floor(random(1, 100));
  // V Auto módu držíme spawn na normálních hodnotách, aby to nezničilo PC
  ...
  for (let i = 0; i < endNotes.length; i++) {
    setTimeout(() => {
      if (gameState === "WAITING") {
        fxSynth.play(endNotes[i] + random(-20, 20), 0.08, 0, 0.4);
...
function updateJukebox() {
  ...
  if (millis() > nextNoteTime) {
    ...
    synth.play(pow(2, (note - 69) / 12) * 440, 0.01, 0, 1.5);
...
function draw() {
  ...
  // Kontrola ručního pohybu se slidery
  if (settingsPanelVisible) {
    if (gravitySlider.value() !== currentGravity || bounceSlider.value() !== currentBounce || spawnPerEventSlider.value() !== spawnPerEvent) {
       if (isAutoMode) toggleAutoMode(); // Vypne auto mód při ručním zásahu
  ...
  if (settingsPanelVisible) {
    push();
    fill(0, 0, 30, 220); rect(20, 80, 300, 240, 12);
    fill(0, 255, 255); textSize(14); text("TEST SETTINGS 🔧", 40, 105);
  ...
  stroke(0, 220, 255, 140); strokeWeight(2); rect(W/2 - 140, 6, 280, 28, 8);
...
function spawnBall(userName) {
  ...
  balls.push({
    body: ballBody, name: userName, color: leaderboard[userName].color, scored: false, combo: 0, lastHitTime: 0
  });
...
function drawBalls() {
  for (let i = balls.length - 1; i >= 0; i--) {
    ...
    // Zvýraznění kuličky, pokud má kombo
    if (b.combo > 2) {
        fill(255); noStroke(); rect(-7, -7, 14, 14); // Bílý obrys komba
    }
    
    fill(b.color); stroke(255); strokeWeight(1); rect(-5, -5, 10, 10);
    rotate(-b.body.angle);
    fill(b.color); noStroke(); textAlign(CENTER); textSize(8);
    text(b.name, 0, -12);
    ...
    if (pos.y > H - ZONE_H - 10) {
      ...
      if (!b.scored) {
        ...
        if (cz) {
          b.scored = true;
          // Započítání komba do skóre (každý hit = +10% ke skóre)
          ...
          if (finalScore >= 5000) { shakeAmount = 8; playJackpotSound(); }
...
function updateUserLikes(username, count) {
  ...
  if (obj) {
    ...
    obj.lastActiveTime = millis(); obj.alpha = 255;
  } else {
...
function drawViewerObjects() {
  ...
  for (let obj of viewerSpaceObjects) {
    ...
    if (obj.img) { imageMode(CENTER); tint(255, obj.alpha); image(obj.img, 0, 0, totalS, totalS); }
      ...
      fill(obj.color[0], obj.color[1], obj.color[2], obj.alpha); ellipse(0, 0, totalS);
      fill(255, obj.alpha); textAlign(CENTER, CENTER); textSize(totalS * 0.3); text(obj.name[0], 0, 0);
    }
...
function drawUI() {
  ...
  textAlign(LEFT, CENTER); fill(0, 255, 255, 20); textSize(64); text(GAME_TITLE, logoX + 4, logoY + 4);
  fill(255); textSize(64); text(GAME_TITLE, logoX, logoY);
  fill(0, 255, 255); textSize(11); text("STABLE SINGULARITY SIMULATION v5.5.0", logoX + 2, logoY + 34);
  ...
  fill(0, 255, 255); textSize(10); text("GEOMETRY: PROCEDURAL | DATA: SYNCED", dropZoneX + dropZoneW/2, 55);
  
  fill(0, 255, 255); textAlign(RIGHT); textSize(9); text(`${currentDestination}`, W - 25, 25);
  let gDisp = floor(map(currentGravity, 0.05, 1.95, 1, 99)); fill(200); textSize(8);
  ...
  if (gameState === "PLAYING") {
    ...
    textAlign(LEFT, CENTER); fill(255, 200, 0); text("COOLING DOWN...", 22, 18);
  ...
  fill(0, 255, 255); textAlign(CENTER); textSize(10); text("TOP CONTRIBUTORS", 125, 20);
...
function initGame() {
  ...
  if (mode.startsWith("SHAPE_")) {
    ...
    for (let i = 0; i < numPegs; i++) {
      ...
      if (valid) {
        let isExplosive = random() < 0.05; // 5% šance na výbušný peg
        let peg = Matter.Bodies.circle(px, py, 2.5, { isStatic: true, restitution: pegRestitution, collisionFilter: { category: 2 } });
        peg.isExplosive = isExplosive;
        pegs.push(peg); Matter.World.add(world, peg);
      }
...
function drawPegs() {
  ...
  for (let p of pegs) {
    ...
    // Vykreslení výbušných pegů
    if (p.isExplosive) {
        fill(255, 100, 0); // Oranžová barva pro výbušninu
        rect(p.position.x - 3, p.position.y - 3, 6, 6);
    } else {
        fill(pegBaseCol);
...
function handleBlackHole() {
  ...
  if (audioStarted) {
    let centerDist = abs(W/2 - blackHole.x); let tremolo = map(sin(frameCount * 0.2), -1, 1, 0.8, 1.0);
    let vol = map(centerDist, W, 0, 0, 0.08) * tremolo; bhOsc.amp(vol, 0.1); bhOsc.freq(32 + n * 12);
  }
  ...
  for (let i = balls.length - 1; i >= 0; i--) {
    ...
    if (d < blackHole.size * 1.87) {
      let safeDist = Math.max(d, 30); let forceDir = Matter.Vector.sub({x: blackHole.x, y: blackHole.y}, b.body.position);
      let strength = (blackHole.size * 0.00018) / (safeDist / 80); let force = Matter.Vector.mult(Matter.Vector.normalise(forceDir), strength);
...
function resetGame() {
  leaderboard = {}; totalBallsFired = 0; roundCount++; gameState = "PLAYING"; resultsTimer = 10; eventOccurredThisRound = false;
  currentDestination = generatePlanetName();
  timer = floor(random(40, 181));
  
  if (isAutoMode) {
      autoRandomSettings(); // Pokud je zapnutý AUTO mód, samo se to teď na začátku kola přelosuje
  }
...
function triggerCosmicEvent() {
  ...
  eventOccurredThisRound = true; let fromLeft = random() < 0.5; let size = random(25, 45); let startX = fromLeft ? -100 : W + 100;
...
function handleCosmicEvent() {
  ...
  for(let i = 0; i < cosmicEvent.trail.length; i++) {
    let alpha = map(i, 0, cosmicEvent.trail.length, 0, 150); fill(red(cosmicEvent.color), green(cosmicEvent.color), blue(cosmicEvent.color), alpha);
