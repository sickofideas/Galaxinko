...
// NOVA PROMENNA: pro vizualni meteory
...
function initTTS() {
  if ('speechSynthesis' in window) {
    let setV = () => { availableVoices = window.speechSynthesis.getVoices(); }; window.speechSynthesis.onvoiceschanged = setV; setV();
  }
}
...
function getHoustonStory(p) {
  const i = ["Houston here. ", "Command center. "], a = [`Energy surge from `, `Maneuvers by `];
...
function speakName(n) {
  ...
  } else if (/[àáâèéêìíîòóôùúû]/i.test(n)) {
    v = availableVoices.find(x => x.lang && x.lang.includes('fr'));
...
function setup() {
  ...
  // OPRAVA: Casovac vizualnich meteoru na 66 sekund
...
function draw() {
  ...
  drawGravityDust(); drawGalacticBackground(); drawViewerObjects();
  
  // OPRAVA: Vykresleni vizualnich meteoru v pozadi
  ...
  if (gameState === "PLAYING") {
    // OPRAVA: Spousteni po 66 vterinach
    if (millis() > nextMeteorShowerTime) { triggerMeteorShower(); nextMeteorShowerTime = millis() + 66000; }
  }
...
// OPRAVA: Vizualni meteor shower misto fyzickych kulicek
...
// NOVA FUNKCE PRO VIZUALNI METEORY
...
function drawUI() {
  ...
  textSize(9); fill(0, 150); text("STABLE SINGULARITY SIMULATION v10.0", lX + 2, 54 + 2); fill(255); text("STABLE SINGULARITY SIMULATION v10.0", lX, 54);
...
// --- GALAXINKO v10.0 (OPRAVA DLE GALAXINKO.txt) - ČÁST 4/4 ---

// OPRAVA: Milestones se aktualizuji pri kazdem zasahu
function checkAllTimeRecords(n, s, c) {
  let i = allTimeRecords.findIndex(r => r.name === n);
  if (i !== -1) {
    if (s > allTimeRecords[i].score) allTimeRecords[i].score = s;
  } else {
...
// OPRAVA: Propojeni na checkAllTimeRecords pri pricitani bodu
...
function spawnRareLegend() {
  ...
  spaceDebris.push({
    x: random(50, W - 50), y: -100, type: "LEGEND", legendId: l.id, size: l.size,
    color: color(l.col[0], l.col[1], l.col[2]), speed: random(0.8, 1.8),
    vx: random(-0.5, 0.5), rot: random(TWO_PI), rotSpeed: random(-0.06, 0.06),
    wobble: random(0.02, 0.08), isRare: true
  });
}
...
function initGame() {
  ...
  const mode = random(p);
  let nP = floor(random(300, 450)), pR = map(currentBounce, 1, 99, 0.1, 1.8);
  ...
  if (mode.startsWith("SHAPE_")) {
    ...
    for (let r = 0; r < rws; r++) {
      for (let c = 0; c < cls; c++) {
        if (sh[r][c] === '*') { let pg = Matter.Bodies.circle(sx + c * sp + random(-1, 1), sy + r * sp + random(-1, 1), 4, { isStatic: true, restitution: pR, collisionFilter: { category: 2 } }); pg.isExplosive = random() < 0.04; pg.isRepulsor = !pg.isExplosive && random() < 0.04; pegs.push(pg); Matter.World.add(world, pg); }
      }
    }
  } else {
    for (let i = 0; i < nP; i++) {
  ...
  for (let i = 0; i < 21; i++) {
    ...
    zones.push({ x: cX, w: zW, score: val, flash: 0, flashColor: color(255), baseColor: val >= 5000 ? color(50, 45, 15, 180) : color(10, 10, 40, 180) });
...
function resetGame() {
  ...
  pegs = []; walls = []; balls = []; blackHole = null; cosmicEvent = null; shootingStars = []; ambientComets = []; portals = []; floatingTexts = []; shockwaves = [];
  initGame(); generateDeepSpaceElements(); prepareSingularityEvents(); planSpaceshipForRound();
  
  // OPRAVA: Resetovani casovace meteoritu pri novem kole
  nextMeteorShowerTime = millis() + 66000;
